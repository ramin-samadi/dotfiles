#!/bin/bash
sudo pacman -Syu wget pavucontrol pamixer playerctl firefox unzip xorg nodejs tldr lsd flatpak ncdu btop bash-completion traceroute tree trash-cli cronie vi neovim linux-headers electron
...
paru -Syu appimagelauncher nmap whois calcurse spotify rustdesk-bin motrix-bin amdguid-glow-bin vulkan-amdgpu-pro lib32-vulkan-amdgpu-pro
...
function open-ports(){
    sudo nmap -sS -D 192.168.0.80 "$1" | grep -e 'PORT' -e 'open'
}
