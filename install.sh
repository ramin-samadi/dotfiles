...
if  [[ $USER == "ramin" ]]; then
    ...
    $aur -Syu tree-sitter-html-git tree-sitter-javascript-git tree-sitter-css-git bash-language-server tree-sitter-bash-git luarocks composer php javacc java julia tree-sitter polkit-kde-agent ruby balena-etcher npm yarn pnpm appimagelauncher fd lazygit libguestfs openrazer-daemon openrazer-driver-dkms openrazer-meta w3m python-openrazer polychromatic bc ufw fail2ban ngrok jq mpv ueberzug ffmpeg ffmpeg4.4 yt-dlp dust ripgrep fzf ueberzug dust nmap whois amdguid-glow-bin vulkan-amdgpu-pro lib32-vulkan-amdgpu-pro
